...
function o() {
	// Capture arguments
	#region
	var subject, explicit_scope, _method, argc, args;
	switch (argument_count) {
		...
		case 1: //o([subject, method, <args>]) form
			...
			if (is_array(argument[0])) {
				argc = array_length(call_block)-2;
				if (argc >= 0) {
					subject = call_block[0];
					_method = call_block[1];
	...
	if (is_array(_method)) {
		if (array_length(_method) == 2) {
			explicit_scope = _method[1];
			_method = _method[0];
		} else {
	...
	global.__fakeoop_name__ = _method;
	...
	if (is_undefined(explicit_scope)) { //Default mode
		...
		with (subject) {
			if (script_execute(mixin_scope)) {
				show_error("Cannot find method " + string(_method) + " in mixin " + script_get_name(mixin_scope) + ".", true);
			}
		}
	}
...
#macro Args array_length(global.__fakeoop_args__)
#macro FromClass , 0+
#macro FromMixin , -1-1*
